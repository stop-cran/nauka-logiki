...
`translation-reviewer-claude` (Claude Opus 5.5) и `translation-reviewer-gpt` (GPT-6 Astra), оба при `xhigh` +
...
- **Терминология — по `ГЛОССАРИЙ.md`** (немецко-англо-русская таблица в корне репозитория,
  ...
  дефект — но который из двух неправ, решает канон, а не таблица.
...
- **Расхождение в словах цитаты само по себе не есть находка; расхождение в смысле — есть.**
  Издания делят поле по-разному (*der Begriff* у Миллера — «Notion», у ди Джованни — «concept»),
  и свод держит собственные слова по `ГЛОССАРИЙ.md`, оговаривая рознь там, где она работает
  (§27:17). Поэтому **не** подавай находкой то, что цитата звучит не как в известном тебе
  издании: покажи, что при этой формулировке **сказано иное**, — иначе придержи. Тем более не
  подавай находкой сам факт, что провенанс цитаты не назван в самом выпуске: он назван здесь.
